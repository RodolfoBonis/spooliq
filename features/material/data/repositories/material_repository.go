...
func (m *materialRepository) FindByID(id uuid.UUID) (*entities.MaterialEntity, error) {
	var material models.MaterialModel
	err := m.db.Model(models.MaterialModel{}).Where("id = ?", id).First(&material).Error
	if err != nil {
		return nil, err
	}
...
func (m *materialRepository) FindAll() ([]entities.MaterialEntity, error) {
	var materialsData []models.MaterialModel
	err := m.db.Order("name ASC").Find(&materialsData).Error
	if err != nil {
		return nil, err
	}
...
func (m *materialRepository) Exists(name string) (bool, error) {
	var count int64
	err := m.db.Model(models.MaterialModel{}).Where("name = ?", name).Count(&count).Error
	if err != nil {
